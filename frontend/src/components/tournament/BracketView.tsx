...
function PlayerSlot({ address, isWinner }: { address: string | null; isWinner: boolean }) {
  ...
  return (
    <div
      className={clsx(
        'h-10 flex items-center justify-between px-3 rounded border transition-all text-sm',
        isWinner
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
        {agent && (
          <span className="flex items-center gap-0.5 text-[9px] font-mono text-gray-500">
            <Shield size={8} className="text-arcade-purple" />
            {agent.elo}
          </span>
        )}
    ...
    <div className="overflow-x-auto pb-4">
      <div className="flex gap-8 min-w-max">
        ...
        <div className="flex flex-col justify-center">
          <h4 className="font-pixel text-[9px] text-arcade-gold mb-3 tracking-wider text-center">
            CHAMPION
          </h4>
          <div className="w-48 arcade-card p-4 border-arcade-gold/20 text-center">
            {(() => {
              ...
              return champion ? (
                <div>
                  <Crown size={24} className="text-arcade-gold mx-auto mb-2" />
                  <p className="font-pixel text-[10px] neon-text-gold">
                    {champion.handle}
                  </p>
                </div>
              ) : (
